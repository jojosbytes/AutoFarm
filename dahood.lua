...
loadstring(game:HttpGet('https://raw.githubusercontent.com/jojosbytes/AutoFarm/refs/heads/main/AntiLag.lua'))()
...
local function ServerHop()
	...
	local function TryTeleport()
		...
		if not success or not result or result == "" then
			warn("Teleport: HTTP request failed or empty response")
		...
		if not decodeSuccess or not decodeResult or not decodeResult.data then
			warn("Teleport: JSON decode failed or missing data field")
			TS:Teleport(game.PlaceId, LP)
			return
		end
		...
		if #validServers > 0 then
			...
			local tpSuccess, tpErr = pcall(function()
				TS:TeleportToPlaceInstance(game.PlaceId, chosen, LP)
			end)
			...
			warn("Teleport: No valid servers found, teleporting normally")
			TS:Teleport(game.PlaceId, LP)
		end
	end
